function updateSubPrices() {
  if (subPrices.length > 1) {
    for (let i = 0; i < subPrices.length; i++) {
      console.log(calcP(subPrices[i]));
      _('subprice_' + i).innerHTML = calcP(subPrices[i]);
    }
  }
}
...
if (_('buyCP')) {
  _('buyCP').addEventListener('click', function buyProduct(e) {
    ...
    let printMat = _('printMat').value;
    let paths = arr.map(v => v.split('/')[2].replace('.stl', ''));

    window.location.href =
    `/buy?product=cp&rvas=${rvas}&suruseg=${suruseg}&color=${color}&scale=${scale}&fvas=${fvas}&q=${quantity}&printMat=${printMat}&files=${paths}`;
